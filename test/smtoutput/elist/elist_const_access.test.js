...
describe ("SMT Exec -- elist decl and access", () => {
    it("should smt exec simple elist", function () {
        runishMainCodeUnsat('public function main(): Int { return (|2i, true|).0; }', "(assert (not (= #x02 Main@main)))"); 
    ...
    it("should smt exec option elist", function () {
        runishMainCodeUnsat('public function main(): Int { let x: Option<(|Int, Bool|)> = some((|2i, true|)); return x@some.0; }', "(assert (not (= (@Result-ok #x02) Main@main)))"); 
    });
});
